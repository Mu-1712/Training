...
create table s1.persons( 
		...
		products varchar(25), country varchar(25) not null,

		price float, years int
);
go
...
select *, Row_number() over (order by price) as row_num from s1.persons; 
go
...
Row_number() over (partition by years order by price) 
as row_num 
from s1.persons;
go
...
select * from (select Row_number() OVER(ORDER BY price)
as row_name, Pname, products, price
from s1.persons) P WHERE row_name > 3 and row_name <= 6;
go